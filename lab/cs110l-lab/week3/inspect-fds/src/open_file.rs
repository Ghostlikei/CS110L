...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let name_path = binding.to_str()?;
        let name = OpenFile::path_to_name(&name_path);

        let info_path = fs::read_to_string(format!("/proc/{}/fdinfo/{}", pid, fd)).ok()?;
        let cursor = OpenFile::parse_cursor(&info_path).unwrap();
        let access_mode = OpenFile::parse_access_mode(&info_path).unwrap();

        let res: OpenFile = OpenFile::new(name, cursor, access_mode);
        Some(res)
    }
